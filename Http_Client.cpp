...
size_t write_data_binary(void *ptr, size_t size, size_t nmemb, Data_Info* data_info) {
	size_t len = size * nmemb;
  if((int)(data_info->data_len + len) <= data_info->file_size + 4) {
	  memcpy(data_info->data + data_info->data_len, ptr, len);
	  data_info->data_len += len;
    }
  return len;
}
...
CURLcode Http_Client::post(const std::string & str_url, const std::string & str_post, std::string & str_response) {
	CURLcode res;
...
CURLcode Http_Client::get(const std::string & str_url, std::string & str_response) {
	CURLcode res;
	CURL* curl = curl_easy_init();
	if(NULL == curl) {
		return CURLE_FAILED_INIT;
...
std::string Http_Client::upload_byte_file(const std::string &str_url, void* pData, int nSize) {
	if(str_url.empty())
		return "";
...
bool Http_Client::download_byte_file(const std::string &str_url, Data_Info* data_info) {
	...
	curl_easy_setopt(curl, CURLOPT_WRITEDATA, data_info);
	CURLcode res = curl_easy_perform(curl);

	int retcode = 0;
	res = curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE , &retcode);
	if(CURLE_OK != res || retcode != 200) {
		LIB_LOG_ERROR("curl_easy_perform failed, res=%d, ret=%u", res, retcode);
	}
	double len = 0;
	res = curl_easy_getinfo(curl, CURLINFO_CONTENT_LENGTH_DOWNLOAD , &len);
	curl_easy_cleanup(curl);
	if (len != data_info->file_size) {
		return false;
	}
	return true;
}
